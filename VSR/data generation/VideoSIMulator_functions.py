...
def SIMimages(opt, DIo, DOi, PSFi, PSFo, background,foreground):
    ...
    for i_a in range(opt.Nangles):
        ...
        for i_s in range(opt.Nshifts):
            ...
            # sig = transform.swirl(sig, centre, strength=warp_factor, radius=rad, rotation=0, clip=True)
            # save_image(sig)
            sig_i = DIo[:,:,index]*sig  # In focus fluorescent response         
...
def Generate_SIM_Image(opt, Io, Oi):
    ...
    # To prevent aggressive background removal only add background to 70 percent of training data
    ratio = np.random.rand()
    if  ratio > 0.3:
        background = 0.8+0.2*np.random.rand() # Random background intensity
        foreground = 1
    ...
    DIo = Io.astype('float')
    DOi = Oi.astype('float') 
    output = np.zeros((opt.Nangles*opt.Nshifts+2,w,w)) # Output stack
    frames = SIMimages(opt, DIo, DOi, PSFi, PSFo, background, foreground) # Generate SIM images
    
    # Apply noise to data
    for i in range(opt.Nangles*opt.Nshifts):

        ST = transform.downscale_local_mean(frames[i,:,:], (opt.scale,opt.scale))
        ...
        pFactor = opt.Poisson # Poisson noise factor
        interim = ST*pFactor 
        if np.amax(interim)<400:		
            poissonNoise = np.random.poisson(ST*pFactor*(0.5* np.random.random_sample()+0.5)).astype(float)
            STnoisy = ST + NoiseFrac*nST + poissonNoise
        else:
            STnoisy = ST + NoiseFrac*nST 
               
        output[i,0:STnoisy.shape[0],0:STnoisy.shape[1]] = STnoisy 

    GT = DIo[:,:,1]
    PSFsim = calcPSF(w,1.5*pixelSize,opt.NA,opt.emission,rindexObj,rindexSp,0,0,offset) # In focus PSF
    OTFsim = np.abs(np.fft.fftshift(np.fft.fft2(PSFsim))) # flatten out the OTF equivalent to filtering reconstruction
    GT = (np.abs(ifft2(fft2(GT)*fftshift(OTFsim)))) # In focus signal
    GT = GT/np.amax(GT) # Normalise to 1
    output[frames.shape[0]-1,:,:] = PSFi
