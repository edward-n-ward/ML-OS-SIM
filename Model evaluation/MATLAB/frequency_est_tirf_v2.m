function [ shiftvalue,background ] = frequency_est_tirf_v2(sepFT,suppress_noise_factor,fmask,if_show_factor,cutoff)
...
[xsize,ysize,a_num,p_num]=size(sepFT);
norm_ft=zeros(size(sepFT));
im=norm_ft;
re_f=im;
reference=zeros(xsize,ysize,a_num);
for ii=1:a_num
    for jj=1:p_num
        ft_max=max(max(abs(sepFT(:,:,ii,jj))));
        sepFT(:,:,ii,jj)=sepFT(:,:,ii,jj)./ft_max;
        norm_ft(:,:,ii,jj)=sepFT(:,:,ii,jj)./(suppress_noise_factor+abs(sepFT(:,:,ii,jj)));
...
shiftvalue=zeros(a_num,p_num,2);%the rough estimation of the modulation frequency k0
% phase mask is introduced to provide a better recognition of the frequency k0
for ii=1:a_num
    for jj=1:p_num
        if jj==1
            [shiftvalue(ii,jj,1),shiftvalue(ii,jj,2)]=find(abs(re_f(:,:,ii,jj))==max(max(abs(re_f(:,:,ii,jj)))));
        else
            temp = find(abs(re_f(:,:,ii,jj)).*fmask==max(max(abs(re_f(:,:,ii,jj)).*fmask)));
            if size(temp,1)>1
                [shiftvalue(ii,jj,1),shiftvalue(ii,jj,2)]= ind2sub([xsize ysize],temp(2));
            else
