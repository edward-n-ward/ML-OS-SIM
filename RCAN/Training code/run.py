...
def GetParams():
  ...
  opt.accumulations = 20 # batch size multiplier
  ...
  opt.n_resblocks  = 4 # number of residual blocks 
  opt.n_resgroups  = 4 # number of residual groups 
  opt.reduction  = 16 
  opt.n_feats = 164 # number of feature maps
...
def train(dataloader, validloader, net, nepoch=10):
    ...
    for epoch in range(start_epoch, nepoch):
        mean_loss = 0
        description = 'Epoch: %d/%d' % (epoch+1,nepoch-start_epoch)
